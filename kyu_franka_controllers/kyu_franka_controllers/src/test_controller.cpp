#include "kyu_franka_controllers/test_controller.h"
...
TestController::command_interface_configuration() const 
{
...
controller_interface::InterfaceConfiguration
TestController::state_interface_configuration() const 
{
...
CallbackReturn TestController::on_activate(const rclcpp_lifecycle::State& /*previous_state*/) 
{
  ...
  if (!async_calculation_thread_.joinable()) {
    async_calculation_thread_ = std::thread(&TestController::runCalculationWorker, this);
  }
  update_joint_states();
  updateRobotData();
...
controller_interface::CallbackReturn TestController::on_deactivate(const rclcpp_lifecycle::State& /*previous_state*/) 
{
...
void TestController::runCalculationWorker()
{
  while (true) {
    {
...
void TestController::asyncCalculationProc()
{
...
void TestController::update_joint_states() {
...
void TestController::updateRobotData()
{
...
Eigen::MatrixXd TestController::getGravity(const Eigen::VectorXd& q)
{
    if(q.size() != model_.nq)
    {
...
Eigen::MatrixXd TestController::getEEJac(const Eigen::VectorXd& q)
{
    if(q.size() != model_.nq)
    {
        std::cerr << "getEEJac Error: size of q " << q.size() << " is not equal to model.nq size: " << model_.nq << std::endl;
...
Eigen::Matrix4d TestController::getEEPose(const Eigen::VectorXd& q)
{
    if(q.size() != model_.nq)
    {
...
Eigen::MatrixXd TestController::getLinkJac(const Eigen::VectorXd& q, const std::string& link_name)
{
    if(q.size() != model_.nq)
    {
        std::cerr << "getEEJac Error: size of q " << q.size() << " is not equal to model.nq size: " << model_.nq << std::endl;
...
Eigen::Matrix4d TestController::getLinkPose(const Eigen::VectorXd& q, const std::string& link_name)
{
    if(q.size() != model_.nq)
    {
        std::cerr << "getEEPose Error: size of q " << q.size() << " is not equal to model.nq size: " << model_.nq << std::endl;
...
void TestController::JointPDControl(const Eigen::Vector7d target_q)
{
...
// NOLINTNEXTLINE
PLUGINLIB_EXPORT_CLASS(kyu_franka_controllers::TestController,
