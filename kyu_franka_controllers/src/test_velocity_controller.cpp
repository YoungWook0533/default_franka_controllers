#include "kyu_franka_controllers/test_velocity_controller.h"
...
TestVelocityController::command_interface_configuration() const 
{
...
controller_interface::InterfaceConfiguration
TestVelocityController::state_interface_configuration() const 
{
...
CallbackReturn TestVelocityController::on_activate(const rclcpp_lifecycle::State& /*previous_state*/) 
{
  ...
  if (!async_calculation_thread_.joinable()) {
    async_calculation_thread_ = std::thread(&TestVelocityController::runCalculationWorker, this);
  }
  update_joint_states();
  updateRobotData();
...
void TestVelocityController::runCalculationWorker()
{
  while (true) {
    {
...
void TestVelocityController::asyncCalculationProc()
{
...
void TestVelocityController::update_joint_states() {
...
Eigen::MatrixXd TestVelocityController::getGravity(const Eigen::VectorXd& q)
{
    if(q.size() != model_.nq)
    {
...
Eigen::MatrixXd TestVelocityController::getEEJac(const Eigen::VectorXd& q)
{
    if(q.size() != model_.nq)
    {
        std::cerr << "getEEJac Error: size of q " << q.size() << " is not equal to model.nq size: " << model_.nq << std::endl;
...
Eigen::Matrix4d TestVelocityController::getEEPose(const Eigen::VectorXd& q)
{
    if(q.size() != model_.nq)
    {
...
Eigen::MatrixXd TestVelocityController::getLinkJac(const Eigen::VectorXd& q, const std::string& link_name)
{
    if(q.size() != model_.nq)
    {
        std::cerr << "getEEJac Error: size of q " << q.size() << " is not equal to model.nq size: " << model_.nq << std::endl;
...
Eigen::Matrix4d TestVelocityController::getLinkPose(const Eigen::VectorXd& q, const std::string& link_name)
{
    if(q.size() != model_.nq)
    {
        std::cerr << "getEEPose Error: size of q " << q.size() << " is not equal to model.nq size: " << model_.nq << std::endl;
...
void TestVelocityController::JointPDControl(const Eigen::Vector7d target_q)
{
...
// NOLINTNEXTLINE
PLUGINLIB_EXPORT_CLASS(kyu_franka_controllers::TestVelocityController,
