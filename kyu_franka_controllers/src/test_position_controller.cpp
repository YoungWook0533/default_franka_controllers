#include "kyu_franka_controllers/test_position_controller.h"
...
TestPositionController::command_interface_configuration() const 
{
...
controller_interface::InterfaceConfiguration
TestPositionController::state_interface_configuration() const 
{
...
CallbackReturn TestPositionController::on_activate(const rclcpp_lifecycle::State& /*previous_state*/) 
{
  ...
  if (!async_calculation_thread_.joinable()) {
    async_calculation_thread_ = std::thread(&TestPositionController::runCalculationWorker, this);
  }
  update_joint_states();
  updateRobotData();
...
void TestPositionController::runCalculationWorker()
{
  while (true) {
    {
...
void TestPositionController::asyncCalculationProc()
{
...
void TestPositionController::update_joint_states() {
...
Eigen::MatrixXd TestPositionController::getGravity(const Eigen::VectorXd& q)
{
    if(q.size() != model_.nq)
    {
...
Eigen::MatrixXd TestPositionController::getEEJac(const Eigen::VectorXd& q)
{
    if(q.size() != model_.nq)
    {
        std::cerr << "getEEJac Error: size of q " << q.size() << " is not equal to model.nq size: " << model_.nq << std::endl;
...
Eigen::Matrix4d TestPositionController::getEEPose(const Eigen::VectorXd& q)
{
    if(q.size() != model_.nq)
    {
...
Eigen::MatrixXd TestPositionController::getLinkJac(const Eigen::VectorXd& q, const std::string& link_name)
{
    if(q.size() != model_.nq)
    {
        std::cerr << "getEEJac Error: size of q " << q.size() << " is not equal to model.nq size: " << model_.nq << std::endl;
...
Eigen::Matrix4d TestPositionController::getLinkPose(const Eigen::VectorXd& q, const std::string& link_name)
{
    if(q.size() != model_.nq)
    {
        std::cerr << "getEEPose Error: size of q " << q.size() << " is not equal to model.nq size: " << model_.nq << std::endl;
...
// NOLINTNEXTLINE
PLUGINLIB_EXPORT_CLASS(kyu_franka_controllers::TestPositionController,
