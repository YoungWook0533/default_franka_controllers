#include "dyros_fr3_controllers/default_controller.h"
...
CallbackReturn DefaultController::on_init() 
{
  try 
  {
    ...
    control_mode_sub_ = get_node()->create_subscription<std_msgs::msg::Int32>(
      "default_controller/control_mode",
      rclcpp::QoS(10),
      std::bind(&DefaultController::controlModeCallback, this, std::placeholders::_1)
);
...
controller_interface::InterfaceConfiguration DefaultController::command_interface_configuration() const 
{
...
controller_interface::InterfaceConfiguration DefaultController::state_interface_configuration() const 
{
...
controller_interface::return_type DefaultController::update(const rclcpp::Time& /*time*/, const rclcpp::Duration& period)
{
  ...
  Eigen::Vector7d last_torque;
  {
    std::lock_guard<std::mutex> lk(calculation_mutex_);
    last_torque = torque_desired_;
  }
...
void DefaultController::updateJointStates() 
{
  std::lock_guard<std::mutex> lk(robot_data_mutex_);
  for (int i = 0; i < num_joints; ++i) 
  {
...
void DefaultController::setMode(CtrlMode mode)
{
  std::lock_guard<std::mutex> lk(calculation_mutex_);
...
// =========================== ROS Subs & Pubs  ===========================
// ========================================================================
void DefaultController::controlModeCallback(const std_msgs::msg::Int32& msg)
{
...
// ========================== Utility Functions ===========================
// ========================================================================	
Eigen::Vector7d DefaultController::JointPDControl(const Eigen::Vector7d target_q, const Eigen::Vector7d target_qdot)
{
...
// NOLINTNEXTLINE
PLUGINLIB_EXPORT_CLASS(dyros_fr3_controllers::DefaultController,
